...
from scripts import stt_nemotron_backend as backend
from scripts import stt_nemotron_probe as probe
...
class ProvenanceTests(unittest.TestCase):
    ...
    def test_display_policy_is_shared_with_simulstreaming(self):
        adapter = TranscriptAdapter(NemotronConfig())
        self.assertEqual(simul_backend.VISIBLE_TEXT_MAX_CHARS, adapter.display_max_chars)
        self.assertEqual(simul_backend.DISPLAY_LINE_MAX_CHARS, adapter.line_max_chars)
...
class TranscriptAdapterTests(unittest.TestCase):
    ...
    def test_long_utterance_is_rolled_up_not_sent_whole(self):
        ...
        for event in events:
            self.assertLessEqual(len(event["text"]), simul_backend.VISIBLE_TEXT_MAX_CHARS)
