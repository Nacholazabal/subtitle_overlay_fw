...
// open/ioctl/close syscalls redirected to fakes, so the frame-count contract is
// exercised without a real /dev/hdmi-vdma device. System headers are included
// before the redirect macros so their prototypes stay intact.
...
// Bring `dma` to an open, ready-to-use state for the API tests below.
static void open_dma(uint32_t frame_count)
{
    TEST_ASSERT_EQUAL_INT(XST_SUCCESS, video_dma_init(&dma, frame_count));
}

void test_init_claims_requested_frames_when_kernel_has_enough(void)
{
    fake_kernel_frame_count = 3U;

    TEST_ASSERT_EQUAL_INT(XST_SUCCESS, video_dma_init(&dma, 3U));
...
void test_init_fails_when_kernel_exposes_fewer_frames_than_requested(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(XST_FAILURE, video_dma_init(&dma, 3U));
}
...
void test_init_fails_when_kernel_reports_no_buffers(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(XST_FAILURE, video_dma_init(&dma, 2U));
}
...
{
    TEST_ASSERT_EQUAL_INT(XST_INVALID_PARAM, video_dma_init(NULL, 2U));
    TEST_ASSERT_EQUAL_INT(XST_INVALID_PARAM, video_dma_init(&dma, 0U));
    TEST_ASSERT_EQUAL_INT(XST_INVALID_PARAM, video_dma_init(&dma, VIDEO_DMA_MAX_FRAMES + 1U));
}
...
void test_cleanup_closes_device_and_resets_state(void)
{
    open_dma(3U);
...
void test_configure_mm2s_passes_geometry_to_kernel(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(
        XST_SUCCESS,
        video_dma_configure(&dma, VIDEO_DMA_CHANNEL_MM2S, 1920U, 1080U, 7680U, 1U));
...
void test_configure_s2mm_uses_s2mm_request(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(
        XST_SUCCESS,
        video_dma_configure(&dma, VIDEO_DMA_CHANNEL_S2MM, 1280U, 720U, 5120U, 0U));
...
void test_configure_reports_ioctl_failure(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(
        XST_FAILURE,
        video_dma_configure(&dma, VIDEO_DMA_CHANNEL_MM2S, 1920U, 1080U, 7680U, 0U));
}
