...
# Nemotron 3.5 / NeMo cache-aware streaming launcher. Same board->PC->Colab->firmware
# path and the same bridge as run_stt_colab_stream.sh and run_stt_colab_simulstream.sh;
# only the Colab STT engine differs. Segmentation and end-of-utterance detection run
# in Colab (official NeMo RNNTGreedyEndpointing), so the faster-whisper tuning env vars
# are intentionally NOT forwarded here — the Nemotron tuning travels as a single
# generic backend-config JSON.
