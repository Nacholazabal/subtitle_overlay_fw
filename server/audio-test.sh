...
# Produces reports and an overlay reconstruction using the final backend.
#
#   ./server/audio-test.sh                 # one normal run + report
#   ./server/audio-test.sh --offline-only  # just build offline proxies
#   ./server/audio-test.sh --sweep         # confirmatory parameter sweep
#
# Override the Colab endpoint (defaults to the shared static ngrok tunnel):
#   STT_STREAM_URL="wss://.../stt/stream" ./server/audio-test.sh
#
...
exec python3 -m server.audio_tests.short_audio "$@"
