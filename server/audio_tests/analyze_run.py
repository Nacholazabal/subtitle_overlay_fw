...
Usage:
    python3 -m server.audio_tests.analyze_run [--jsonl logs/stt_events.jsonl] [--wav logs/board_audio.wav]
"""
...
def analyze_config(events):
    ...
    print(f"  language     : {config.get('config_target_lang')}")
    print(f"  decoder      : {config.get('config_decoder_type')}")
    print(f"  lookahead    : {config.get('config_latency_ms')} ms")
    print(f"  context      : {config.get('config_att_context_size')}")
    print(f"  EOU history  : {config.get('config_stop_history_eou_ms')} ms")
    print(f"  residue      : {config.get('config_residue_tokens_at_end')} token(s)")
