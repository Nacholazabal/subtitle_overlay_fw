...
from server.runtime import nemotron as nemotron_backend
...
# Nemotron session tuning uses the generic backend-config JSON transport.
# ``latency_ms`` is the model's published operating point
...
NEMOTRON_PROFILE = Profile(
    ...
    sweep_file=REPO_ROOT / "server" / "audio_tests" / "sweeps" / "nemotron_confirmatory.json",
...
PROFILES = {NEMOTRON_PROFILE.name: NEMOTRON_PROFILE}


def select_profile(name: str | None) -> Profile:
    name = name or os.environ.get("AUDIO_TEST_PROFILE") or NEMOTRON_PROFILE.name
    try:
...
def requested_session_config(args, profile: "Profile" = None) -> dict:
    profile = profile or NEMOTRON_PROFILE
    requested = {
        name: value
...
def offline_signature(
    ...
    # Include the engine identity so caches remain tied to the final backend.
...
def overlay_timeline(events: list[dict]) -> list[dict]:
    from server.audio_tests.subtitle_debug import SubtitleModel
...
def backend_metrics(events: list[dict], done: dict, audio_duration_sec: float | None) -> dict:
    """Streaming-quality metrics for the Nemotron runtime.
...
def make_report(run_dir: Path, manifest: dict, ready: dict, done: dict) -> dict:
    ...
    for clip_index, clip in enumerate(clips):
        ...
        # Prefer the complete segment kept in full_text over its bounded
        # firmware-facing rolling tail.
        live_text = " ".join(
    ...
    run_engine = (
        ...
        or nemotron_backend.RUN_ENGINE
    )
...
def compare_runs() -> int:
    ...
    if not reports:
        ...
        return 0
    header = (
        f"{'RUN':<24} {'MODEL':<24} {'LOOK':>6} {'EOU':>6} {'RES':>4} "
    ...
    for report in reports:
        ...
        delivery = report.get("global_metrics", {}).get("board_delivery", {})
        latency = report.get("global_metrics", {}).get("latency", {})
        model = Path(str(config.get("config_model", "?"))).name
        print(
            f"{report.get('run_id', '?'):<24} {model:<24} "
            f"{fmt(config.get('config_latency_ms'), 0):>6} "
            f"{fmt(config.get('config_stop_history_eou_ms'), 0):>6} "
            f"{fmt(config.get('config_residue_tokens_at_end'), 0):>4} "
            ...
            f"{str(delivery.get('accepted', '?')) + '/' + str(delivery.get('generated', '?')):>11} "
...
def run_benchmark(args) -> int:
    ...
    bridge = subprocess.Popen(
        [str(REPO_ROOT / "server" / profile.launcher)],
        cwd=REPO_ROOT,
        env=environment,
        stdout=bridge_log,
...
def load_sweep_cases(path: Path | None = None, profile: "Profile" = None) -> list[dict]:
    ...
    if path is None:
        ...
        if profile.sweep_file is not None:
            ...
            raise ValueError(f"profile {profile.name!r} has no default sweep file")
    if path is not None:
...
def run_sweep(args) -> int:
    ...
    sweep = {
        ...
        "status": "running",
        "profile": profile.name,
        "run_engine": profile.expected_engine,
        "cases": cases,
        "runs": [],
    }
...
def parse_args() -> argparse.Namespace:
    ...
    # Nemotron session parameters (optional single-run overrides).
    ...
    try:
        requested_session_config(args, NEMOTRON_PROFILE)
