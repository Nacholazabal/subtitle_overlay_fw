#!/usr/bin/env python3
"""FastAPI / WebSocket server exposing the final Nemotron streaming backend.

The bridge, session protocol and firmware ACK path expose:
...
class BackendState:
    ...
    def health_payload(self) -> dict:
        ...
        payload = {
            # ``ready`` and ``run_engine`` are the authoritative state fields.
...
def create_app(config: ServerConfig, *, backend_state=None):
    ...
    async def stt_stream(websocket: WebSocket):
        ...
        try:
            ...
            validate_session_start(session_start)
