...
Final STT engine selected for the subtitle overlay thesis. It wraps the
*official* NeMo streaming inference stack
...
# Firmware line limits, caption roll-up and PCM conversion are engine-independent.
from server.runtime.captions import (
...
# ``backend_config`` field (see ``server.runtime.protocol``). Validation is
# server-side so the transport remains independent from model-specific flags.
_CONFIG_FIELDS = {
...
class NemotronConfig:
    ...
    def run_config(self, *, realtime: bool, transport: str) -> dict:
        """Return the effective, traceable configuration for this run."""
...
def take_line_words(words: list[str], max_chars: int) -> int:
    ...
    The adapter promotes text to finished lines by *word count*, so the cut must
    land on a word boundary; a character cut would lose the tail of the word."""
    if not words:
        return 0
    total = 0
    count = 0
...
class TranscriptAdapter:
    ...
    Responsibilities required by the firmware and bridge:
    ...
    * ``is_final=True`` only when a display line is completed — because the
    ...
    def stats_snapshot(self) -> dict:
        return {
            ...
            # Every emitted event is delivered and no event is discardable, so
            # jobs == events and the evaluation can score pipeline reliability.
...
def transcribe_offline_float32(shared_model: SharedNemotronModel, audio, config: NemotronConfig) -> dict:
    ...
    from server.evaluation.probe import NemotronProbeConfig, build_offline_transcribe_config
